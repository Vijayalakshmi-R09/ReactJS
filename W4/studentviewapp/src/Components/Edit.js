...
export default function Edit() {
    ...
    useEffect(()=>{
        ...
        axios.get("http://localhost:4001/records").then(res => {
          console.log(res);
          setResult(res.data);
        }).catch(e=>{
    ...
    const handleDelete=(e)=>{
        e.preventDefault();
          ...
          console.log(id);
          const url=` http://localhost:4001/records/${id}`
          axios.delete(url)
          .then(res1 => {
            console.log(res1);
            setRes(res1.data);
            ...
            <table className="table table-striped table-dark">
                ...
                <tbody>
                   {res.map((x)=>{
                    ...
                    <td><Link  className="btn btn-primary active" to={`/edit/edituser/${x.id}`}>Edit</Link></td>
